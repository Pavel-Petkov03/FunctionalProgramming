...
(define (null-graph?) null?)

; мйсля че тази гнус може да се обходи в дълбочина и ширина ама ще видим

(define (cons#f current-v a b)
  (cond
    [a (cons current-v a)]
    [b (cons current-v b)]
    [else
...
(define (path graph v u)
  (cond
    ...
    [else
     (cons#f
      v
      (path (remove-assoc-ref graph v) (graph-ref  v) u) ; в дълбочина
      (path (remove-current-path graph v) (car (graph-ref v)) u) ; в ширина
      )
     ]
    )
  )

(define (remove-assoc-ref graph v)
  (filter (lambda (ls) (not (= (car ls) v ))) graph)
  )

(define (remove-current-path graph v)
  (map (lambda (ls) (if (= v (car ls)) (cons (car ls) (cddr ls)) ls)
         (cond (null? (cdr ls)))
         ) graph)
  )
