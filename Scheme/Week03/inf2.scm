...
(define (argmin f a b)
  (accumulate-i (lambda (res cur)
                ...
                (lambda (x) x)
                (lambda (x) (+ x 1))
                ) ; това е доста неоптимално защото всеки бът смята текущата най малка стойност
...
(define (count-digits number)
  ...
  ) ; за да използвам тая страшно неподходяща функция
    ; ще си сложа condition във a или b за да станат равни като искам да спре
    ; това е задачата иначе това се пише на два реда
